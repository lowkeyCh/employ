<script>
  export default {
    mounted() {
      ...
      this.$axios.get('http://localhost:8081/enterprise/query_all').then(res=>{
        for(let i = 0; i < res.data.length; i++) {
           ...
           * 根据企业id获取企业用人资料相关信息
           */
          this.$axios.get('http://localhost:8081/eq/query_by_enterprise_id?enterpriseId='+ this.enterpriseList[i].enterpriseId).then(res2=>{
            for(let j = 0; j < res2.data.length; j++) {
              this.enterpriseQualifications[i][j] = {
               ...
               */
              this.tableDataList[i].enterpriseId = this.enterpriseList[i].enterpriseId
              this.tableDataList[i].enterpriseName = this.enterpriseList[i].enterpriseName
              this.tableDataList[i].enterpriseQualificationsCheck = this.enterpriseList[i].enterpriseQualificationsCheck
              this.tableDataList[i].eqIndexs[j] = "http://localhost:8081/" + this.enterpriseQualifications[i][j].eqIndex
            }
          })
        }
      })
    ...
    methods: {
       ...
       * 界面展示 同时设置图片路径
       */
      enterpriseQualificationsShow(tableData) {
        this.preview.previewDialog = true
        for(let i = 0; i < tableData.eqIndexs.length; ++i) {
          this.preview.previewImageUrls[i] = tableData.eqIndexs[i]
        }
      },
      ...
      checkPass(tableData) {
        for(let i = 0; i < this.tableDataList.length; ++i) {
          if(tableData.enterpriseId == this.tableDataList[i].enterpriseId) {
            this.tableDataList[i].enterpriseQualificationsCheck = "审核通过"
            this.enterpriseList[i].enterpriseQualificationsCheck = "审核通过"
            /**
             * 发送请求更新审核状态
             */
            this.$axios.post('http://localhost:8081/enterprise/modify', this.enterpriseList[i]).then(res=>{
      ...
      checkOut(tableData) {
        for(let i = 0; i < this.tableDataList.length; ++i) {
          if(tableData.enterpriseId == this.tableDataList[i].enterpriseId) {
            this.tableDataList[i].enterpriseQualificationsCheck = "审核未通过"
            this.enterpriseList[i].enterpriseQualificationsCheck = "审核未通过"
            /**
             * 发送请求更新审核状态
             */
            this.$axios.post('http://localhost:8081/enterprise/modify', this.enterpriseList[i]).then(res=>{
      ...
      checkReset(tableData) {
        for(let i = 0; i < this.tableDataList.length; ++i) {
          if(tableData.enterpriseId == this.tableDataList[i].enterpriseId) {
            this.tableDataList[i].enterpriseQualificationsCheck = "未审核"
            this.enterpriseList[i].enterpriseQualificationsCheck = "未审核"
            /**
             * 发送请求更新审核状态
             */
            this.$axios.post('http://localhost:8081/enterprise/modify', this.enterpriseList[i]).then(res=>{
...
<template>
  <div>
    ...
    >
      ...
      <el-table-column width="300" prop="enterpriseName">
        <template slot="header">
          ...
          <label style="margin-left: 5px;">企业名称</label>
        </template>
      </el-table-column>

      <el-table-column width="300" prop="enterpriseQualificationsCheck">
        <template slot="header">
          ...
          <label style="margin-left: 5px;">企业用人资格审查状态</label>
        </template>
      </el-table-column>

      <el-table-column width="150" prop="eqIndexs">
        <template slot="header">
        ...
        <template slot-scope="scope">
          <el-tooltip class="item" effect="dark" content="用人资格详情" placement="top">
            <el-button style="margin-left: 30px" type="primary" icon="el-icon-view" @click="enterpriseQualificationsShow(scope.row)"></el-button>
          </el-tooltip>
        </template>
      </el-table-column>
      ...
      <el-table-column>
        <template slot="header">
          ...
          <template slot-scope="scope">
            <el-tooltip class="item" effect="dark" content="审核通过" placement="top" v-if="scope.row.enterpriseQualificationsCheck == '未审核'">
              <el-button type="primary" icon="el-icon-check" circle @click="checkPass(scope.row)"></el-button>
            </el-tooltip>

            <el-tooltip class="item" effect="dark" content="审核未通过" placement="top" v-if="scope.row.enterpriseQualificationsCheck == '未审核'">
              <el-button type="danger" icon="el-icon-close" circle @click="checkOut(scope.row)"></el-button>
            </el-tooltip>

            <el-tooltip class="item" effect="dark" content="重置审核状态" placement="top" v-if="scope.row.enterpriseQualificationsCheck != '未审核'">
